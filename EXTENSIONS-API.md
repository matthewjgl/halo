# Halo Extension SDK — JavaScript API

## Context

Halo is a native macOS app (Swift/SwiftUI, macOS 14+) that transforms the MacBook notch area into an interactive island. It currently has 7 built-in modules: Now Playing, Volume HUD, Battery, Connectivity, Calendar, Weather, and Notifications.

We want to make Halo **hackable and extensible** — allowing the community to build, distribute, and install third-party extensions using **JavaScript/TypeScript**, similar to how Raycast extensions work. Extensions run inside a JavaScriptCore sandbox and describe their UI declaratively — the host app renders them natively in SwiftUI.
...
```json
{
  ...
  "author": {
    "name": "Jane Developer",
    "url": "https://github.com/jane/pomodoro-halo"
  },
...
- `notifications` — send macOS notifications and read mirrored notification feed via `Halo.system`
- `storage` — persist extension-scoped key/value state
- `network` — make requests through `Halo.http.fetch()`
- `usage` — read local Codex and Claude usage summaries through `Halo.system.getAIUsage()`

`capabilities.notificationFeed`:
- When `true`, the extension is not shown as a separate module in island cycling.
- `Halo.island.activate()` targets the shared Notifications module.
- `Halo.notifications.send(...)` is mirrored into the shared Halo notifications feed.
...
final class ExtensionJSRuntime {
    ...
    private func injectAPI() {
        // The Halo global namespace
        let di = JSValue(newObjectIn: context)!
        context.setObject(di, forKeyedSubscript: "Halo" as NSString)

        // Halo.registerModule(config)
        ...
        // Halo.store — persistent key-value storage
        ...
        // Halo.http — sandboxed network (only if "network" permission granted)
        ...
        // Halo.timers — setInterval/setTimeout
...
#### 1.4 Extension JavaScript API (`Halo` global)

This is what extension developers use. The following is the **TypeScript type definition** shipped as `@vedit/halo-sdk`:

```typescript
// @vedit/halo-sdk — TypeScript definitions
...
```typescript
// Shipped as part of @vedit/halo-sdk — injected into JSContext as `View` global
...
Shared components are also injected on `Halo.components`. The reusable reply/input tray is available as:

```typescript
Halo.components.inputComposer({
  placeholder: "Reply",
  action: "submit",
  id: "reply-box",
  minHeight: 46,
...
- **Controlled network** — only `Halo.http.fetch()` (proxied through Swift `URLSession`)
...
final class ExtensionSandbox {
    static func configureContext(_ context: JSContext, permissions: [String]) {
        ...
        // Only inject Halo.http if "network" permission granted
        if !permissions.contains("network") {
            // Halo.http.fetch will throw "Permission denied"
        }
...
final class ExtensionManager: ObservableObject {
    ...
    /// ~/Library/Application Support/Halo/Extensions/
...
```json
// registry.json — hosted at https://extensions.halo.vedit.com/registry.json
{
  "version": 1,
  "extensions": [
    {
      ...
      "downloadURL": "https://github.com/jane/pomodoro-halo/releases/download/v1.2.0/pomodoro.zip",
      "iconURL": "https://raw.githubusercontent.com/jane/pomodoro-halo/main/assets/icon.png",
      "downloads": 1523,
      "rating": 4.8,
      "size": 45000,
      "checksum": "sha256:abc123...",
      "screenshots": [
        "https://raw.githubusercontent.com/jane/pomodoro-halo/main/screenshots/compact.png",
        "https://raw.githubusercontent.com/jane/pomodoro-halo/main/screenshots/expanded.png"
      ],
...
```json
{
  ...
  "scripts": {
    "dev": "halo dev",
    "build": "halo build",
    "publish": "halo publish"
  },
  "devDependencies": {
    "@vedit/halo-sdk": "^1.0.0",
    "@vedit/halo-cli": "^1.0.0",
    "typescript": "^5.0.0"
  }
}
```
...
The CLI communicates with the running Halo app via a local Unix socket or Bonjour:
...
final class ExtensionDevServer {
    ...
    let socketPath = "/tmp/halo-dev.sock"
...
- `JSContext` has no filesystem access by default — all storage goes through `Halo.store`
...
function timerCompleted() {
  ...
  if (!isBreak) {
    sessionsCompleted++;
    Halo.store.set("sessionsCompleted", sessionsCompleted);
  }

  isBreak = !isBreak;
  const breakDuration = (Halo.settings.get("breakDuration") || 5) * 60;
  const workDuration = (Halo.settings.get("workDuration") || 25) * 60;
  totalDuration = isBreak ? breakDuration : workDuration;
  remaining = totalDuration;

  if (Halo.settings.get("notifyOnComplete") !== false) {
    Halo.notifications.send({
      title: isBreak ? "Time for a break!" : "Break's over!",
      body: isBreak ? `You've completed ${sessionsCompleted} sessions` : "Let's focus!",
      sound: Halo.settings.get("playSound") !== false,
    });
  }
...
Halo.registerModule({
  onActivate() {
    sessionsCompleted = Halo.store.get("sessionsCompleted") || 0;
    totalDuration = (Halo.settings.get("workDuration") || 25) * 60;
    remaining = totalDuration;
  },
...
2. **JSContext bridge** — Create `ExtensionJSRuntime` with injected `Halo` global, `View` helpers, timers, console
...
5. **Storage & settings** — `Halo.store`, `Halo.settings`, settings schema renderer
...
9. **CLI tool** — `@vedit/halo-cli` for create, dev, build, publish
...
- Node.js APIs (`fs`, `path`, `crypto`, etc.) — only the injected Halo globals
