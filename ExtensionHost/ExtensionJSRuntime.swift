...
final class ExtensionJSRuntime {
    ...
    private func injectAPI() {
        let haloGlobal = JSValue(newObjectIn: context)!
        context.setObject(haloGlobal, forKeyedSubscript: "Halo" as NSString)

        injectModuleRegistration(into: haloGlobal)
        injectStore(into: haloGlobal)
        injectSettings(into: haloGlobal)
        injectIslandControls(into: haloGlobal)
        injectNotifications(into: haloGlobal)
        injectHTTP(into: haloGlobal)
        injectSystem(into: haloGlobal)
        injectFeedback(into: haloGlobal)
        injectMascot(into: haloGlobal)
        injectConsole(into: haloGlobal)
        injectTimers()
        injectViewHelpers()
        injectComponents()
    }

    private func injectModuleRegistration(into haloGlobal: JSValue) {
        ...
        haloGlobal.setObject(registerModule, forKeyedSubscript: "registerModule" as NSString)
    }
    ...
    private func injectStore(into haloGlobal: JSValue) {
        ...
        haloGlobal.setObject(store, forKeyedSubscript: "store" as NSString)
    }

    private func injectSettings(into haloGlobal: JSValue) {
        ...
        haloGlobal.setObject(settings, forKeyedSubscript: "settings" as NSString)
    }

    private func injectIslandControls(into haloGlobal: JSValue) {
        ...
        haloGlobal.setObject(island, forKeyedSubscript: "island" as NSString)
    }

    private func injectNotifications(into haloGlobal: JSValue) {
        ...
        haloGlobal.setObject(notifications, forKeyedSubscript: "notifications" as NSString)
    }

    private func injectHTTP(into haloGlobal: JSValue) {
        ...
        haloGlobal.setObject(fetchSync, forKeyedSubscript: "__fetchSync" as NSString)

        if manifest.permissions.contains("network") {
            context.evaluateScript(
                "Halo.http = { fetch: function(url, options) { return Promise.resolve(Halo.__fetchSync(url, options)); } };"
            )
        } else {
            context.evaluateScript(
                "Halo.http = { fetch: function() { throw new Error('Permission denied: network'); } };"
            )
        }
    }

    private func injectSystem(into haloGlobal: JSValue) {
        ...
        haloGlobal.setObject(system, forKeyedSubscript: "system" as NSString)
    }

    private func injectFeedback(into haloGlobal: JSValue) {
        ...
        haloGlobal.setObject(playFeedback, forKeyedSubscript: "playFeedback" as NSString)
        haloGlobal.setObject(openURL, forKeyedSubscript: "openURL" as NSString)
    }

    private func injectMascot(into haloGlobal: JSValue) {
        ...
        haloGlobal.setObject(mascot, forKeyedSubscript: "mascot" as NSString)
    }

    private func injectConsole(into haloGlobal: JSValue) {
        ...
        haloGlobal.setObject(logInfo, forKeyedSubscript: "__log" as NSString)
        haloGlobal.setObject(logWarn, forKeyedSubscript: "__warn" as NSString)
        haloGlobal.setObject(logError, forKeyedSubscript: "__error" as NSString)

        context.evaluateScript(
            """
            globalThis.console = {
              log: function() { Halo.__log(Array.from(arguments).map(String).join(' ')); },
              warn: function() { Halo.__warn(Array.from(arguments).map(String).join(' ')); },
              error: function() { Halo.__error(Array.from(arguments).map(String).join(' ')); }
            };
            """
        )
    }
    ...
    private func injectComponents() {
        context.evaluateScript(
            ...
            (function() {
              ...
              const existing = Halo.components || {};
              Halo.components = {
                ...existing,
                shortcutHint,
    ...
    private func sendNotification(
        ...
        if shouldShowSystemNotification {
            ...
            let request = UNNotificationRequest(
                identifier: "halo.\(extensionID).\(UUID().uuidString)",
                content: content,
