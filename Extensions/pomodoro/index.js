...
function settingNumber(key, fallback) {
  return toNumber(Halo.settings.get(key), fallback);
}

function settingBool(key, fallback) {
  var value = Halo.settings.get(key);
...
function resetSessionsIfNewDay() {
  var storedDate = Halo.store.get("sessionsDate");
  var today = todayDateString();
  if (storedDate !== today) {
    sessionsCompleted = 0;
    Halo.store.set("sessionsCompleted", 0);
    Halo.store.set("sessionsDate", today);
  }
}
...
function saveState() {
  Halo.store.set("phase", phase);
  Halo.store.set("isRunning", isRunning);
  Halo.store.set("remainingSeconds", remainingSeconds);
  Halo.store.set("sessionsCompleted", sessionsCompleted);
  Halo.store.set("sessionsDate", todayDateString());
}

function loadState() {
  resetSessionsIfNewDay();
  sessionsCompleted = toNumber(Halo.store.get("sessionsCompleted"), 0);
  var storedPhase = Halo.store.get("phase");
  phase = storedPhase === PHASE_BREAK ? PHASE_BREAK : PHASE_FOCUS;
  var storedRemaining = toNumber(Halo.store.get("remainingSeconds"), currentPhaseDuration());
  remainingSeconds = Math.max(0, Math.min(storedRemaining, currentPhaseDuration()));
  var storedRunning = Halo.store.get("isRunning");
...
function isFullExpanded() {
  return Halo.island.state === "fullExpanded";
}
...
function revealIsland() {
  ...
  setTimeout(function() { Halo.island.activate(false); }, revealSettleDelay);
  setTimeout(function() { Halo.island.dismiss(); }, visibleDuration + revealSettleDelay);
}
...
function updateMascotExpression() {
  if (!isRunning || phase === PHASE_BREAK) {
    Halo.mascot.setExpression("idle");
    return;
  }
  Halo.mascot.setExpression("working");
}
