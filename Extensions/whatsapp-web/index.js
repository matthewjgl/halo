...
function renderInputComposer(options) {
  if (Halo.components && typeof Halo.components.inputComposer === "function") {
    return Halo.components.inputComposer(options);
  }
...
function shouldShowConnectionHint() {
  const configured = Halo.settings.get("showConnectionHint");
...
function refreshState(force) {
  ...
  if (typeof Halo.system.getWhatsAppWeb !== "function") {
    return;
  }

  const snapshot = asObject(Halo.system.getWhatsAppWeb(8));
  if (!snapshot) return;
...
function openReplyComposer(payload) {
  ...
  if (typeof Halo.system.startWhatsAppWeb === "function") {
    Halo.system.startWhatsAppWeb();
  }
  refreshState(true);
}
...
function mediaPreviewSection() {
  const previewText = replyComposer.preview || "Send a quick reply from Halo.";
...
Halo.registerModule({
  ...
  onAction(actionID) {
    if (actionID === "refresh-qr" && typeof Halo.system.refreshWhatsAppWebQR === "function") {
      Halo.system.refreshWhatsAppWebQR();
      refreshState(true);
      return;
    }
    ...
    if (actionID === "close-reply") {
      closeReplyComposer();
      const closed = typeof Halo.system.closePresentedInteraction === "function"
        ? !!Halo.system.closePresentedInteraction()
        : false;
      if (!closed) {
        refreshState(true);
      }
    ...
    if (actionID === "submit-reply") {
      ...
      if (typeof Halo.system.sendWhatsAppWebMessageAsync !== "function") {
      ...
      Halo.system.sendWhatsAppWebMessageAsync(recipient, body);
      if (notificationSourceID && typeof Halo.system.dismissNotification === "function") {
        Halo.system.dismissNotification(notificationSourceID);
      }
      closeReplyComposer();
      const closed = typeof Halo.system.closePresentedInteraction === "function"
        ? !!Halo.system.closePresentedInteraction()
