...
Extensions are JavaScript packages that run inside Halo's sandboxed runtime. They can render UI in the compact pill, the expanded drawer, and the full detail panel — and run background logic to fetch or compute data.
...
|---|---|
| `storage` | `Halo.store` key-value persistence |
| `network` | `Halo.http.fetch()` |
...
The `Halo` global is injected before your script runs.
...
// Register your hooks:
Halo.extension.onInit(onInit)
Halo.extension.onRefresh(onRefresh)
Halo.extension.onAction(onAction)
Halo.extension.onSettingsChanged(onSettingsChanged)

// --- Storage ---

Halo.store.set("key", "value")   // persist a value
Halo.store.get("key")             // retrieve it (returns null if not set)

// --- Settings ---

Halo.settings.get("myKey")        // read a value from settings.json schema

// --- Network ---

Halo.http.fetch("https://api.example.com/data")
...
Halo.island.activate()    // bring the island to the foreground
Halo.island.dismiss()     // collapse back to compact
```
...
function onInit() {
  symbol = Halo.settings.get("symbol") || "AAPL";
  render();
}

function onRefresh() {
  Halo.http.fetch("https://query1.finance.yahoo.com/v8/finance/quote?symbols=" + symbol)
    .then(function(res) {
...
- Persist any state you'd want restored across app restarts using `Halo.store`.
