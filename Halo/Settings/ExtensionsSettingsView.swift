...
private let linearMentionsExtensionID = "halo.linear-mentions"
        ...
        if let manifest = selectedManifest {
            ScrollView {
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    if manifest.id == "halo.whatsapp-web" {
...
private struct LinearOAuthSettingsView: View {
    private static let authorizeURLString = "https://api.supercmd.sh/auth/linear/authorize?app=halo"
    ...
    private var statusMessage: String {
        if let session {
            ...
            return "Linear is authenticated. New mentions will appear in the Halo."
        }
