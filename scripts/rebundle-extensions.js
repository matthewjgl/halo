...
//
// Wipes stale extensions out of every Halo.app bundle on this machine
...
//                                                     # also wipe ~/Library/Application Support/Halo/Extensions
...
const APP_SUPPORT_DIR = path.join(os.homedir(), "Library", "Application Support", "Halo", "Extensions");
...
function findAppBundles() {
  ...
  const localBuild = path.join(REPO_ROOT, "build", "Halo.app");
  if (fs.existsSync(localBuild)) found.add(localBuild);

  // 2. Installed app
  const installed = "/Applications/Halo.app";
  ...
  if (fs.existsSync(DERIVED_DATA)) {
    for (const entry of fs.readdirSync(DERIVED_DATA)) {
      if (!entry.startsWith("Halo-")) continue;
      ...
      for (const cfg of fs.readdirSync(products)) {
        const app = path.join(products, cfg, "Halo.app");
...
if (!apps.length) {
  warn("No Halo.app bundles found. Build the app first or pass --app <path>.");
  process.exit(1);
}
...
log(dryRun ? "Dry run complete." : "Restart Halo to see the changes.");
